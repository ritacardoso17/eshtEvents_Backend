        ...
        if (!error) {
            ...
            bcrypt.compare(pass, rows[0].password, function (error, results) {
                ...
                if (results) {
                    ...
                    callback(null, {id: rows[0].id_utilizador, typeUser: rows[0].id_tipoUser, imgProfile:rows[0].foto_perfil, password: pass, name: rows[0].nome, email: email, birthDate: rows[0].data_nascimento, school: rows[0].school , token: token})
                }
            })
        }
        else {
