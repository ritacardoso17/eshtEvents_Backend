...
function getReservationsId(id, callback) {
    connection 
    const sql = "SELECT id_extra, id_utilizador, nr_pessoas, data_hora_reserva, data_hora_evento, id_farda, tipo_reserva.descritivo as tipoReserva,id_estado, id_menu, id_localizacao, id_decoracao, opiniao FROM reserva_evento,tipo_reserva WHERE id_reserva=? AND tipo_reserva.id_tipo_reserva = reserva_evento.id_tipo_reserva"
    connection.query(sql, [id], function (error, results) {
        if (error) callback(error)
