...
function addWorkshops(nome,description, n_vacancies, date_hour,locutor, id_local, img, callback) {
    const sql = "INSERT INTO inscricao_workshop (nome,descritivo, nr_vagas, data_hora, locutor, id_localizacao, img) VALUES (?,?,?,?,?,?,?)"
    connection.query(sql, [nome,description, n_vacancies, date_hour,locutor, id_local, img], function (error, results) {
        if (!error){
